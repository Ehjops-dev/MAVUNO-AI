...
children.push(
  ...
  p('Finally, the administrator console (Figure 4.9) gives the operator a platform-wide view — portfolio analytics, the farmer register, the credit book and an audit trail of every administrative write — reached through the same sign-in form, separated only by the role stored against the account.'),
  ...figure('shot_admin.png', 'Figure 4.9: Administrator Control Centre', 620, 386),
  h2('4.4 Description of Testing and Results'),
  h3('4.4.1 System Testing'),
  p('Testing combined an automated suite of 86 tests written with the Node built-in test runner — which spawns the real server on an isolated port with a temporary database and exercises every API endpoint, the scoring engine, input validation, static-file security and concurrency — with a nine-flow manual user-acceptance walkthrough performed in the browser. The suite is reproducible with a single command: node --test tests/api.test.js.'),
